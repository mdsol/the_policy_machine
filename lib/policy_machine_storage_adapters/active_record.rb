...
unless active_record_unavailable
    ...
    class ActiveRecord
      ...
      def self.const_missing(name)
        if %w[Assignment LogicalLink Adapter].include?(name.to_s)
          load_db_adapter!
          "::PolicyMachineStorageAdapter::ActiveRecord::#{name}".constantize
        else
          super
        end
      end
      ...
      def self.persist_buffers!
        ...
        buffers[:upsert].each_value { |el| el.attributes = el.attributes.slice(*column_keys) }
      ...
      def self.class_for_type(pe_type)
        @pe_type_class_hash ||= Hash.new do |h, k|
          h[k] = "PolicyMachineStorageAdapter::ActiveRecord::#{k.camelize}".constantize
        end
      ...
      class PolicyElement < ApplicationRecord
        ...
        %w[
          ...
          define_method("pluck_from_#{graph_method}") do |fields:, filters: {}|
            raise ArgumentError, 'Must provide at least one field to pluck' unless fields.present?
        ...
        def pluck_ancestor_tree(fields:, filters: {})
          raise ArgumentError, 'Must provide at least one field to pluck' unless fields.present?
          ...
          fields_to_pluck = %i[id unique_identifier] | fields
        ...
        def self.bulk_destroy(elements)
          id_groups = elements.each_with_object(Hash.new { |h, k| h[k] = [] }) do |(_, el), memo|
        ...
        def self.bulk_assign(pairs_hash)
          ...
          Assignment.import(%i[parent_id child_id], id_pairs, on_duplicate_key_ignore: true)
        end
        ...
        def self.bulk_link(pairs_hash)
          id_pairs = pairs_hash.values.map do |parent, child|
            [parent.id, child.id, parent.policy_machine_uuid, child.policy_machine_uuid]
          end
          import_fields = %i[link_parent_id link_child_id link_parent_policy_machine_uuid
                             link_child_policy_machine_uuid]
          LogicalLink.import(import_fields, id_pairs, on_duplicate_key_ignore: true)
        end

        def self.bulk_associate(associations, _upsert_buffer)
          ...
          PolicyElementAssociation.import(associations,
        ...
        def select_relative_attributes(id_attribute_tree, relative_ids)
          relative_ids.each_with_object([]) do |relative_id, attribute_array|
            ...
            if relative_attributes.is_a?(Hash)
      ...
      class PolicyElementAssociation < ApplicationRecord
        ...
        DUPLICATE_KEY_UPDATE_PARAMS = { conflict_target: %i[user_attribute_id object_attribute_id operation_set_id],
                                        index_predicate: 'operation_set_id IS NOT NULL',
                                        columns: %i[user_attribute_id object_attribute_id operation_set_id] }.freeze
      ...
      POLICY_ELEMENT_TYPES.each do |pe_type|
        ...
        define_method("add_#{pe_type}") do |unique_identifier, policy_machine_uuid, extra_attributes = {}|
          ...
          buffering? ? klass.create_later(element_attrs, self) : klass.create(element_attrs)
        end
      ...
      def assign(src, dst)
        ...
        if buffering?
          ...
          Assignment.import(%i[parent_id child_id], [[src.id, dst.id]], on_duplicate_key_ignore: true)
        end
      end

      def assign_later(parent:, child:)
        buffers[:assignments][[parent.unique_identifier, child.unique_identifier]] = [parent, child]
        :buffered
      end
      ...
      def link(src, dst)
        ...
        if buffering?
          ...
          LogicalLink.import(
            %i[link_parent_id link_child_id link_parent_policy_machine_uuid link_child_policy_machine_uuid],
      ...
      def link_later(parent:, child:)
        buffers[:links][[parent.unique_identifier, child.unique_identifier]] = [parent, child]
        :buffered
      end
      ...
      def unassign(src, dst)
        buffering? ? unassign_later(src, dst) : unassign_now(src, dst)
      end
      ...
      #
      def unlink(src, dst)
        buffering? ? unlink_later(src, dst) : unlink_now(src, dst)
      end
      ...
      #
      def delete(element)
        buffering? ? delete_later(element) : element.destroy
      end
      ...
      def update(element, changes_hash)
        ...
        buffering? ? update_later(element) : element.save
      end
      ...
      def is_privilege?(user_or_attribute, operation, object_or_attribute)
        ...
        if policy_classes_containing_object.size < 2
          ...
          policy_classes_containing_object.all? do |_policy_class|
            !accessible_operations(user_or_attribute, object_or_attribute, operation_id).empty?
          end
        end
      end
      ...
      def batch_pluck(policy_object, fields:, query: {}, config: {})
        raise(ArgumentError, 'must provide fields to pluck') unless fields.present?

        method("pluck_all_of_type_#{policy_object}").call(fields: fields,
          options: query).find_in_batches(config) do |batch|
      ...
      def accessible_ancestor_objects(user_or_attribute, operation, root_object, options = {})
        ...
        unless associations_with_operation = options[:associations_with_operation]
          associations = associations_for_user_or_attribute(user_or_attribute,
            options.except(:associations_with_operation))
      ...
      def include_condition?(_key, value)
      ...
      def accessible_operations(user_or_attribute, object_or_attribute, operation_id = nil, filters: {})
        transaction_without_mergejoin do
          ...
          prms[:unique_identifier] = operation_id if operation_id
      ...
      def transaction_without_mergejoin
