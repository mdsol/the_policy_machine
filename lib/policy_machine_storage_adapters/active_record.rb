      ...
      define_method("find_all_of_type_#{pe_type}") do |options = {}|
        ...
        extra_attribute_conditions.each do |key, value|
          ...
          all.select!{ |pe| pe.methodize_extra_attributes_hash and 
            ((attr_value = pe.extra_attributes_hash[key]).is_a?(String) and value.is_a?(String) and options[:ignore_case] == true) ? attr_value.downcase == value.downcase : attr_value == value}
        end
