...
module PolicyMachineStorageAdapter
  ...
  class ActiveRecord
    ...
    class PolicyElement < ::ActiveRecord::Base
      ...
      def pluck_ancestor_attributes_from_ancestors(filters: {}, fields:, min_depth: nil, max_depth: nil)
        assert_valid_attributes!(filters.keys)
        assert_valid_attributes!(fields)
        ...
        #     also passes in the minimum and maximum depth (from root) for filtering results
        id_tree = Assignment.select_ancestor_ids([id], min_depth, max_depth).each_with_object({}) do |row, memo|
