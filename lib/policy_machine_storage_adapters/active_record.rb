  ...
  class ActiveRecord
    ...
    def accessible_operations2(user_or_attribute, object_or_attribute, operation_id = nil, prohibition_id = nil, filters: {})
      transaction_without_mergejoin do
        ...
        Assignment.descendants_of(associations.map(&:operation_set)).where(prms.merge(unique_identifier: operation_id)).or(PolicyMachineStorageAdapter::ActiveRecord::PolicyElement.where(prms.merge(unique_identifier: prohibition_id)))
      end
    end
