...
unless neography_unavailable
  module PolicyMachineStorageAdapter
    class Neography
      POLICY_ELEMENT_TYPES = %w[user user_attribute object object_attribute operation policy_class].freeze
          ...
          found_elts.select do |elt|
            options.except(:ignore_case).all? do |k, v|
              if v.nil?
                ...
                elt.respond_to?(k) && elt.send(k).respond_to?(:include?) && [*v[:include]].all? do |val|
                  elt.send(k).include?(val)
                end
              else
                elt.respond_to?(k) &&
                  (if elt.send(k).is_a?(String) && v.is_a?(String) && ignore_case_applies?(options[:ignore_case],
                    k)
                     elt.send(k).casecmp(v).zero?
                   else
                     elt.send(k) == v
                   end)
              end
      ...
      def delete(element)
        if %w[user_attribute object_attribute].include?(element.pe_type)
          element.outgoing(:in_association).each(&:del)
        end
        element.del
      end
      ...
      def add_association(user_attribute, object_attribute, policy_machine_uuid)
      ...
      def associations_with(operation)
        operation.outgoing(:in_association).map do |association|
          user_attribute = ::Neography::Node.find('nodes', 'unique_identifier',
            association.user_attribute_unique_identifier)
          object_attribute = ::Neography::Node.find('nodes', 'unique_identifier',
            association.object_attribute_unique_identifier)
      ...
      def remove_association(user_attribute, object_attribute, _policy_machine_uuid)
        unique_identifier = user_attribute.unique_identifier + object_attribute.unique_identifier
      ...
      def transaction
        raise NotImplementedError,
          "transactions are only available in neo4j 2.0 which #{self.class} is not compatible with"
      end
