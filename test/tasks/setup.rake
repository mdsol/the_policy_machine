  ...
  namespace :test do
    ...
    task :prepare, [:db_type] do |t, args|
      Dir.chdir('./test') do
        ...
        `rm -rf testapp`
        `bundle exec rails new testapp -f -d #{db_type} -m ./template.rb  --skip-keeps  --skip-git --skip-docker --skip-asset-pipeline --skip-javascript`
      end
      ...
      Dir.chdir('./test/testapp') do
        ...
        `bundle exec rails generate the_policy_machine:accessible_objects_for_operations_function_cte -f`
