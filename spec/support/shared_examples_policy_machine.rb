...
shared_examples 'a policy machine' do
  ...
  describe 'Assignments' do
    allowed_assignments = [
      %w[object object],
      %w[object object_attribute],
      %w[object_attribute object_attribute],
      %w[object_attribute object],
      %w[user user_attribute],
      %w[user_attribute user_attribute],
      %w[user_attribute policy_class],
      %w[object_attribute policy_class],
      %w[operation_set operation_set],
      %w[operation_set operation]
    ]
    ...
    describe 'Adding' do
      ...
      it 'raises when first argument is not in policy machine' do
        ...
        expect { policy_machine.add_assignment(pe0, pe1) }
          .to raise_error(ArgumentError,
            "#{pe0.unique_identifier} is not in policy machine with uuid #{policy_machine.uuid}")
      end
      ...
      it 'raises when second argument is not in policy machine' do
        ...
        expect { policy_machine.add_assignment(pe0, pe1) }
          .to raise_error(ArgumentError,
            "#{pe1.unique_identifier} is not in policy machine with uuid #{policy_machine.uuid}")
      end
    end
    ...
    describe 'Adding' do
      ...
      it 'raises when first argument is not in policy machine' do
        ...
        expect { policy_machine.add_association(ua, @operation_set, @object_attribute) }
          .to raise_error(ArgumentError,
            "#{ua.unique_identifier} is not in policy machine with uuid #{policy_machine.uuid}")
      end
      ...
      it 'raises when the second argument is not in policy machine' do
        ...
        expect { policy_machine.add_association(@user_attribute, @operation_set, oa) }
          .to raise_error(ArgumentError,
            "#{oa.unique_identifier} is not in policy machine with uuid #{policy_machine.uuid}")
      end
      ...
      describe 'in_user_attribute' do
        it 'raises unless options[:in_user_attribute] is a PM::UserAttribute' do
          expect { policy_machine.is_privilege?(@u1, @w, @o2, in_user_attribute: 4) }
            .to raise_error(ArgumentError,
              "expected options[:in_user_attribute] to be a PM::UserAttribute; got #{SmallNumber}")
        end
      ...
      describe 'in_object_attribute' do
        it 'raises unless options[:in_object_attribute] is a PM::ObjectAttribute' do
          expect { policy_machine.is_privilege?(@u1, @w, @o2, in_object_attribute: 4) }
            .to raise_error(ArgumentError,
              "expected options[:in_object_attribute] to be a PM::ObjectAttribute; got #{SmallNumber}")
        end
        ...
        it 'accepts both in_user_attribute and in_object_attribute' do
          project2 = policy_machine.create_object_attribute('Project2')
          expect(policy_machine.is_privilege?(@u1, @w, @o1, 'in_user_attribute' => @group1,
'in_object_attribute' => project2))
            .to be_falsey
        end
      end
    end
  ...
  describe 'The Mail System:  Figure 8. (pg. 43)' do
    ...
    it 'can ignore prohibitions' do
      ...
      ignoring_prohibitions = policy_machine.scoped_privileges(@u2, @in_u2, ignore_prohibitions: true).map do |_, op, _|
        op.unique_identifier
      end
  ...
  describe 'accessible_objects' do
    ...
    it 'lists all objects with the given privilege for the given user' do
      expect(policy_machine.accessible_objects(@u1, @read,
        key: :unique_identifier).map(&:unique_identifier)).to include('one:fish', 'two:fish', 'red:one')
      expect(policy_machine.accessible_objects(@u1, @write,
        key: :unique_identifier).map(&:unique_identifier)).to eq(['red:one'])
    end

    it 'filters objects via substring matching' do
      expect(policy_machine.accessible_objects(@u1, @read, includes: 'fish',
key: :unique_identifier).map(&:unique_identifier)).to match_array(['one:fish', 'two:fish'])
      expect(policy_machine.accessible_objects(@u1, @read, includes: 'one',
key: :unique_identifier).map(&:unique_identifier)).to match_array(['one:fish', 'red:one'])
    end
    ...
    context 'cascading operation sets' do
      ...
      it 'lists all objects with the given privilege for the given user 1 operation set deep' do
        expect(policy_machine.accessible_objects(@u1, @speed_read,
          key: :unique_identifier).map(&:unique_identifier)).to include('one:fish', 'two:fish', 'red:one')
      end

      it 'lists all objects with the given privilege for the given user 2 operation sets deep' do
        expect(policy_machine.accessible_objects(@u1, @speediest_read,
          key: :unique_identifier).map(&:unique_identifier)).to include('one:fish', 'two:fish', 'red:one')
      end
    end
    ...
    context 'with prohibitions' do
      ...
      it 'filters out prohibited objects by default' do
        expect(policy_machine.accessible_objects(@u1,
          @read).map(&:unique_identifier)).to match_array(['two:fish', 'red:one'])
      end

      it 'can ignore prohibitions' do
        expect(policy_machine.accessible_objects(@u1, @read,
          ignore_prohibitions: true).map(&:unique_identifier)).to match_array(['one:fish', 'two:fish', 'red:one'])
      end
    end
  end
  ...
  describe 'batch_pluck' do
    ...
    context 'when given a block' do
      it 'calls the block' do
        expect do |spy|
          policy_machine.batch_pluck(
            type: :object, query: { unique_identifier: 'one:fish' }, fields: [:unique_identifier], &spy
          )
        end.to yield_control
      end
      ...
      context 'and search terms' do
        it 'returns the matching attributes' do
          policy_machine.batch_pluck(type: :object, query: { unique_identifier: 'one:fish' },
            fields: [:unique_identifier]) do |batch|
            expect(batch.size).to eq 1
            expect(batch.first[:unique_identifier]).to eq 'one:fish'
          end
        end
        ...
        it 'does not return non-specified attributes' do
          policy_machine.batch_pluck(type: :object, query: { unique_identifier: 'blue:one' },
            fields: [:unique_identifier]) do |batch|
            expect(batch.size).to eq 1
    ...
    context 'when not given a block' do
      ...
      context 'but given search terms' do
        it 'the results are chainable and returns the relevant results' do
          enum = policy_machine.batch_pluck(type: :object, query: { unique_identifier: 'one:fish' },
